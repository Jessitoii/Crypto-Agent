...
import re
class LocalSetFitBrain:
    def __init__(self, model_path="nexus-ai-v2-core"):
        print(f"🔄 Loading Local SetFit Model: {model_path}...")
        self.model = SetFitModel.from_pretrained(model_path)
        # Label map based on training: 0=HOLD, 1=SHORT, 2=LONG
        self.label_map = {0: "HOLD", 1: "SHORT", 2: "LONG"}
        print("✅ Model Loaded Successfully!")

    def analyze(self, text):
        # Tahmin (Probabilities)
        # predict_proba returns [prob_0, prob_1, prob_2]
        probs = self.model.predict_proba([text])[0]
        
        # En yüksek olasılığı bul
        best_idx = torch.argmax(probs.detach()).item()
        confidence = float(probs[best_idx]) * 100  # 0-100 scale
        
        action = self.label_map.get(best_idx, "HOLD")
    ...
    try:
        ...
        if LOCAL_BRAIN_MODE:
            ...
            analysis = await ctx.brain.analyze_specific_no_research(
                news=msg_text_clean,
                symbol=pair,
                price=technicals.get("close", 0.0),
                changes=changes,
                coin_full_name=coin_full_name,
                market_cap_str=cap_str,
                rsi_val=rsi,
                btc_trend=technicals.get("btc_trend", 0.0)
            )
        ...
        if confidence >= 75 and action in ["LONG", "SHORT"]:
            ...
            if LOCAL_BRAIN_MODE:
                ...
                ctx.exchange.open_position_test(
                    symbol=pair, side=action, price=entry_price,
                    tp_pct=analysis['tp_pct'], sl_pct=analysis['sl_pct'],
...
async def run_simulation(model_path="nexus-ai-v2-final"):
    ...
    results_file = "backtest_results_nexus.txt"
...
if __name__ == "__main__":
    ...
    asyncio.run(run_simulation("nexus-ai-v2-final"))